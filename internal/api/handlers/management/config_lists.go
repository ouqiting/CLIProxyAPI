...
func (h *Handler) PutAPIKeyModels(c *gin.Context) {
	...
	var arr []config.APIKeyModelRule
	if err = json.Unmarshal(data, &arr); err != nil {
		var obj struct {
			Items []config.APIKeyModelRule `json:"items"`
		}
		if err2 := json.Unmarshal(data, &obj); err2 != nil || len(obj.Items) == 0 {
			c.JSON(400, gin.H{"error": "invalid body"})
			return
...
func (h *Handler) PatchAPIKeyModels(c *gin.Context) {
	...
	var body struct {
		Index *int              `json:"index"`
		Match *string           `json:"match"`
		Value *apiKeyModelPatch `json:"value"`
	}
	if err := c.ShouldBindJSON(&body); err != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
...
func (h *Handler) GetOpenAICompat(c *gin.Context) {
	...
	for i := range entries {
		for j := range entries[i].APIKeyEntries {
			entries[i].APIKeyEntries[j].Disabled = openaicompat.IsDisabled(
				registry,
				entries[i].Name,
				entries[i].BaseURL,
				entries[i].APIKeyEntries[j].APIKey,
			)
		}
	}
